...
class MotorNApp:
    ...
    def toggle_motor(self):
        """Alterna entre el motor normal y el extendido"""
        self.motor_extendido = not self.motor_extendido
        self.inicializar_motor(self.motor_extendido)
        
        if self.motor_extendido:
            self.btn_toggle_motor.config(text="Desactivar Motor Extendido")
            self.btn_reiniciar_campo.pack(fill=tk.X, pady=2)  # Mostrar botón de reinicio de campo
            self.log_mensaje("Motor extendido activado (con memoria persistente)")
            self.log_mensaje("Prueba a dibujar patrones y luego reiniciar el campo para ver la persistencia")
        else:
    ...
    def inicializar_motor(self, extendido=False):
        """Inicializa el motor normal o extendido según el parámetro"""
        if extendido:
